# **Redis Client Implementation for Nightingale Gateway**
...
### **1. Initializing Redis Client in main.go**
```go
// In main.go
import "nightingale-gateway/internal/cache"

func main() {
    // Simple initialization from URL
    redisClient, err := cache.NewRedisClientFromURL("redis://redis:6379")
    if err != nil {
...
func RateLimitingExample(redisClient *cache.RedisClient) {
    // Rate limiting by IP
    ip := "192.168.1.1"
...
func CachedHandler(redisClient *cache.RedisClient, c *gin.Context) {
    cacheKey := "api:patients:list:" + c.Query("page")
    
    // Check cache first
    var cachedData []Patient
    if err := redisClient.Get(cacheKey, &cachedData); err == nil {
        // Cache hit
    ...
    patients, err := fetchPatientsFromDB()
    if err != nil {
    ...
    redisClient.Set(cacheKey, patients, 5*time.Minute)
    
    c.JSON(http.StatusOK, patients)
}
```

### **4. Using Cache Entries with Metadata**
```go
func CacheWithMetadata(redisClient *cache.RedisClient) {
    data := map[string]interface{}{
        "patients": []string{"John Doe", "Jane Smith"},
        "count":    2,
    }
    
    // Store with metadata
    err := redisClient.SetCacheEntry("patients:cache", data, 10*time.Minute)
    if err != nil {
        // Handle error
    }
    
    // Retrieve with metadata
    entry, err := redisClient.GetCacheEntry("patients:cache")
    if err != nil {
        // Handle error
    }
    
    // Check if cache is still valid
    isValid, _ := redisClient.IsCacheValid("patients:cache")
    if isValid {
        // Use cached data
    }
}
...
```go
func RedisPubSubExample(redisClient *cache.RedisClient) {
    // Subscribe to channel in a goroutine
    go func() {
        pubsub := redisClient.Subscribe("patient:updates")
        defer pubsub.Close()
        
        ch := pubsub.Channel()
        for msg := range ch {
            var update PatientUpdate
    ...
    redisClient.Publish("patient:updates", update)
}
```

## **Testing the Redis Client**

### **Test File: gateway/internal/cache/redis_client_test.go**
```go
package cache
...
func TestRedisClient(t *testing.T) {
	...
	err = client.Set(testKey, testValue, time.Minute)
	assert.NoError(t, err)

	var retrievedValue map[string]interface{}
	err = client.Get(testKey, &retrievedValue)
	assert.NoError(t, err)
	...
	// Test Increment
	count, err := client.Increment("counter")
	assert.NoError(t, err)
	assert.Equal(t, int64(1), count)

	// Test SetNX
	success, err := client.SetNX("nx:key", "value", time.Minute)
	assert.NoError(t, err)
	assert.True(t, success)

	success, err = client.SetNX("nx:key", "another-value", time.Minute)
	assert.NoError(t, err)
...
func TestCacheEntry(t *testing.T) {
	...
	// Test SetCacheEntry and GetCacheEntry
	data := map[string]string{"test": "data"}
	err = client.SetCacheEntry("cache:entry", data, time.Minute)
	assert.NoError(t, err)

	entry, err := client.GetCacheEntry("cache:entry")
	assert.NoError(t, err)
	assert.Equal(t, data, entry.Value)
	assert.NotZero(t, entry.CreatedAt)
	assert.NotZero(t, entry.Expiration)

	// Test IsCacheValid
	valid, err := client.IsCacheValid("cache:entry")
	assert.NoError(t, err)
	assert.True(t, valid)
...
This Redis client implementation provides:
1. **Comprehensive Redis operations** with proper error handling
2. **Connection pooling** for performance
3. **Health checks** and monitoring
4. **Cache entry management** with metadata
5. **Pub/Sub support** for real-time features
6. **Graceful shutdown** and cleanup
7. **Thread-safe operations** with context support
8. **Structured logging** for debugging
9. **Type-safe serialization** with JSON
10. **Prefix-based key management** for organization

The client is ready to be used for rate limiting, caching, session storage, and real-time messaging in the Nightingale Gateway.